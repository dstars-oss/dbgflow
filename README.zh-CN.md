...
安装脚本会先构建 release 版 `dbgflow-mcp`，再调用构建出的 `target\release\dbgflow-mcp.exe service install` 并传入选定的服务参数。安装子命令会复制当前正在运行的 exe 到 `%LOCALAPPDATA%\dbgflow\bin`，以 LocalSystem 和 `service run --data-dir %LOCALAPPDATA%\dbgflow\var` 安装并启动服务，再检查 `/healthz`。服务 artifacts 和 logs 写入 `%LOCALAPPDATA%\dbgflow\var`；卸载子命令默认不删除 artifacts 或 logs。

安装过程中，脚本会尝试识别本机 Sysinternals 目录，并询问是否写入
`--sysinternals-dir` 以启用可选 Procmon 能力。也可以显式传入
`-SysinternalsDir <path>`。如果没有配置 Sysinternals 目录，service 仍会
正常安装运行，但 Procmon-based profiling 不可用。

安装脚本默认为 Windows service 环境配置 `http://127.0.0.1:7897`，包括
`_NT_SYMBOL_PROXY=127.0.0.1:7897`。可使用 `-ProxyUrl <url>` 指定其他代理；
使用 `-NoProxy` 会向 service Environment 写入已知代理键的空值，以覆盖继承的
机器或系统代理环境变量。
