...
或多个 profiling collector。默认 collector 是 `native_etw`，使用
`scope.kind=target_process`、`event_sets=["process_lifecycle"]`，并默认开启
stack capture。它会写入标准 `.etl` trace，并为 target PID 产出过滤后的
`events.jsonl` 和 `summary.json`。工具也接受 `collectors[]` 做并行采集。
...
Native ETW `process_lifecycle` 采集 process start/end、thread start/end 和
image load/unload kernel events。原始 `trace.etl` 仍可能包含系统范围的 lifecycle
事件；dbgflow 后处理输出的 `events.jsonl` 和 `summary.json` 会严格过滤到本次
launch 的 target PID。stack frame 以紧凑字符串数组输出：能匹配 target PID
image load 区间时为 `module+0xoffset`，否则保留原始 `0x...` 地址；不做符号解析。
...
TTD 也是可选依赖；脚本会先从已解析的 DbgEng 目录按 `<dbgeng_dir>\ttd` 推导，
