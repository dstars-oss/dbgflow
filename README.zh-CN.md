...
通过主程序安装或卸载 Windows service；如果当前进程未提权，dbgflow 会弹出 UAC
确认窗口并在确认后继续执行：
...
安装子命令会构建 release binary；如果已存在 `dbgflow-mcp` 服务，则先停止并卸载；然后将 exe 复制到 `%LOCALAPPDATA%\dbgflow\bin`，以 LocalSystem 和 `service run --data-dir %LOCALAPPDATA%\dbgflow\var` 安装并启动服务，再检查 `/healthz`。服务 artifacts 和 logs 写入 `%LOCALAPPDATA%\dbgflow\var`；卸载子命令默认不删除 artifacts 或 logs。
`scripts\` 下的 PowerShell 脚本仍保留为兼容包装入口，会转调这些子命令。
