...
class IdalibSupervisor:
    ...
    def open_session(
        ...
        *,
        ...
        if open_timeout_sec is not None:
            open_timeout = open_timeout_sec or None
        else:
...
def idb_open(
    ...
    open_timeout_sec: Annotated[
        float,
        "Per-open timeout in seconds for headless worker startup and analysis. Uses IDA_MCP_OPEN_TIMEOUT when omitted or zero.",
    ] = 0,
) -> IdalibOpenResult:
    ...
    try:
        session = sup.open_session(
            ...
            open_timeout_sec=open_timeout_sec or None,
        )
        return {
            "success": True,
