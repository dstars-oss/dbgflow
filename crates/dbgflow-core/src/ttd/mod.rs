use crate::artifacts::{ArtifactKind, ArtifactManager, ArtifactRef, TtdRecordingArtifactEvent};
...
pub struct TtdRecordingManager {
    ...
    active_job: Arc<Mutex<Option<TtdRecordingId>>>,
    ...
    pub fn record_ttd(&self, mut request: RecordTtd) -> Result<TtdRecordingResult> {
        ...
        let recording_id = TtdRecordingId::new();
        {
            let mut active = self.active_job.lock().map_err(|_| {
                DbgFlowError::Backend("TTD recording active job lock poisoned".to_string())
            })?;
            if let Some(active_id) = *active {
                let error = DbgFlowError::Backend(format!(
                    "another TTD recording job is already active: {active_id}"
                ));
                self.log_recording_rejected(
                    &request,
                    request_started.elapsed().as_millis(),
        ...
        let active_guard = ActiveTtdRecordingGuard {
...
fn ttd_stop_target(target: &TtdTarget, recorded_pid: Option<u32>) -> Option<OsString> {
    match target {
...
fn split_path_text_has_parent(path: &str) -> bool {
    path.split(['\\', '/']).any(|part| part == "..")
}

fn path_text_has_separator(path: &str) -> bool {
    path.contains('\\') || path.contains('/')
}

fn is_absolute_path_text(path: &Path) -> bool {
    if path.is_absolute() {
        return true;
    }
...
struct ActiveTtdRecordingGuard {
    active_job: Arc<Mutex<Option<TtdRecordingId>>>,
    recording_id: TtdRecordingId,
}

impl Drop for ActiveTtdRecordingGuard {
    fn drop(&mut self) {
        if let Ok(mut active) = self.active_job.lock() {
            if *active == Some(self.recording_id) {
                *active = None;
            }
        }
    }
