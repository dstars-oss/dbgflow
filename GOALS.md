...
已由 D-015/D-016 改为 `target_process + process + file_io + stacks`；
Sysinternals Process Monitor 作为显式启用的 `procmon` collector，用于更高保真的
文件 / 注册表 I/O 事件和可选堆栈 artifact。
...
* 旧 `system_overview` 概览采集已被 D-015 废弃；native ETW 默认承担轻量
  process lifecycle 与常用 file I/O 采集点，不承担完整文件 / 注册表取证。
* Procmon 更适合高保真的文件 / 注册表操作审计和调用栈采集。
...
```text
1. 完成 idalib / IDA SDK 架构设计、runtime 配置和安全边界设计
2. 实现 ReverseBackend / ReverseSessionManager / IdaLibBackend worker spike
3. 设计 `ida.*` tool schema、artifacts 和审计格式
4. 支持 headless reverse session MVP：加载 binary / IDB 并查询函数、反汇编、xrefs 和伪代码
5. 支持受控 IDB 标注修改 MVP：rename、comment 和 type
6. 继续增强 target/path validation、redaction、报告消费格式和真实环境验证
```
