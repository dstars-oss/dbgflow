...
`process_lifecycle` event set、目标退出或 timeout 自动停止采集，并对 target PID
产出后处理后的 `events.jsonl` 和 `summary.json`。
...
`trace.record_profile` 支持 `collectors[]` 并行采集。`native_etw` 默认 collector
已由 D-015 改为 `target_process + process_lifecycle + stacks`；Sysinternals
Process Monitor 作为显式启用的 `procmon` collector，用于更精确的文件 /
注册表 I/O 事件和可选堆栈 artifact。
Procmon collector 保留 `capture.pml` 作为权威 artifact，并导出 `events.csv`
以及按 target PID / operation / path best-effort 过滤后的 `events.jsonl`。

Procmon 只从 `config.toml` 的 `[tools].sysinternals_dir` 中派生
`Procmon64.exe` 或 `Procmon.exe`，不接受单独 exe 路径，不自动下载，不扫描
全盘。安装脚本识别 Sysinternals 目录并写入配置；未配置时，依赖 Sysinternals
的能力不可用。

原因：

* 旧 `system_overview` 概览采集已被 D-015 废弃；文件 / 注册表归因不再由
  native ETW 默认 collector 承担。
* Procmon 更适合文件 / 注册表操作审计和调用栈采集。
* 并行 collector 能在同一次 target 生命周期中同时保留 ETW 概览和 Procmon
  精确 I/O 证据。
* 通过 `config.toml` 显式配置，保持服务行为可审计且不依赖用户环境。
...
读取 `config.toml`。安装脚本负责本机探测 DbgEng、Sysinternals 和 proxy，并写入
...
* 避免 bind、data dir、DbgEng、Sysinternals、proxy 分散在命令行、service
...
* `trace.record_profile` 和 `trace.record_ttd` 都产出敏感 trace artifact，但保留
  独立 schema，避免把 ETW / Procmon profiling 与 TTD recording 生命周期混合。
...
`event_sets=["process_lifecycle"]`，默认开启 stack capture。

`process_lifecycle` 采集侧只启用 process、thread 和 image load kernel flags。
原始 `.etl` 仍可能包含系统范围 lifecycle 事件；PID 精确性由 ETL 后处理产物
`events.jsonl` 和 `summary.json` 保证。stack 输出为紧凑字符串数组，解析成功时
使用 `module+0xoffset`，未解析时保留 raw address；不做符号解析。

原因：

* 避免旧 `system_overview` 采集过宽，难以归因到目标进程。
* event set schema 能自然扩展到后续 file I/O、disk I/O、registry、CPU sample 等能力。
